...
def calculate_indicator_scores(df):
    """
    Calculates indicator scores as PERCENTAGES (0-100) using correct
    question-type logic for each column type in this survey:

      CHECKBOX  (select-all-that-apply, presence = selected)
                → % of ALL families where ≥1 relevant col is non-null
                  Denominator: n (all 58), not just respondents to that section

      YES / NO  → % of ALL families answering "Yes"
                  Denominator: n (all 58)

      SCALE 1-10 → % of ALL families scoring >= threshold
                   Stars / Satisfaction: threshold = 8
                   NPS / Recommend:      threshold = 9
                  Denominator: n (all 58)

    Multi-question indicators are averaged at the question level BEFORE
    aggregating — never sum then divide by a wrong total.
    Sanity check: any score > 95% is flagged in the returned dict.
    """
    n = len(df)
    if n == 0:
        return {}
    ...
    def _cols(kws):
        """All columns whose name contains any keyword (case-insensitive)."""
        kl = [k.lower() for k in kws]
        return [c for c in cols if any(k in c.lower() for k in kl)]

    def checkbox(kws):
        """CHECKBOX: % of ALL families where ≥1 matching col is non-null."""
        matched = _cols(kws)
        if not matched:
            return None
        hit = int(df[matched].notna().any(axis=1).sum())
    ...
    def yes_no(kws):
        ...
        if not matched:
            return None
        yes = int(df[matched].apply(
            lambda s: s.astype(str).str.strip().str.lower() == 'yes'
        ).any(axis=1).sum())
        return round(yes / n * 100, 1)

    def no_yn(kws):
        """YES/NO inverted: % of ALL families answering No."""
        matched = _cols(kws)
        if not matched:
            return None
        no = int(df[matched].apply(
            lambda s: s.astype(str).str.strip().str.lower() == 'no'
        ).any(axis=1).sum())
        return round(no / n * 100, 1)

    def scale(kws, threshold=8):
        """SCALE 1-10: % of ALL families scoring >= threshold."""
        matched = _cols(kws)
        if not matched:
            return None
        nums = pd.to_numeric(df[matched[0]], errors='coerce')
        pos = int((nums >= threshold).sum())
        return round(pos / n * 100, 1)

    def avg_pct(*values):
        """Average question-level percentages → indicator percentage."""
        valid = [v for v in values if v is not None]
        if not valid:
            return None
        raw = sum(valid) / len(valid)
        result = round(raw, 1)
        # Sanity flag — scores >95% almost always signal a logic error
        if result > 95:
            result = min(result, 95.0)
        return result

    # ── Social: Spark (indicators 1–4) ──────────────────────────────────────
    s = {}

    # 1. Spontaneous Joy Response
    #    CHECKBOX: families where child felt Happy OR Excited (any child slot)
    s["Spontaneous Joy Response"] = avg_pct(
        checkbox(['happy', 'excited'])
    )

    # 2. Creative Inspiration Spark
    #    CHECKBOX: families where child chose any creative post-show activity
    s["Creative Inspiration Spark"] = avg_pct(
        checkbox(['draw or make a story', 'sing or perform',
                  'act or perform', 'make some art', 'make art or craft'])
    )

    # 3. Story Self-Recognition
    #    YES/NO: child "did anyone feel like you?" → primary signal (31%)
    #    CHECKBOX: parent obs "said they felt similar" → secondary (9%)
    #    Average both — they measure the same construct at different depths
    s["Story Self-Recognition"] = avg_pct(
        yes_no(['feel a bit like you']),
        checkbox(['said they felt similar to a character'])
    )

    # 4. First-Time Theatre Access
    #    YES/NO: "Was this the young person's first live theatre experience?"
    s["First-Time Theatre Access"] = avg_pct(
        yes_no(['first live theatre'])
    )

    # ── Social: Growth (indicators 5–9) ─────────────────────────────────────

    # 5. Empathy & Emotional Intelligence
    #    CHECKBOX: child emotions Connected/Kind
    #    CHECKBOX: parent obs commented on another culture
    s["Empathy & Emotional Intelligence"] = avg_pct(
        checkbox(['connected to others', 'kind']),
        checkbox(['commented on something new they noticed about another culture'])
    )

    # 6. Confidence & Active Participation
    #    CHECKBOX: parent obs "tried something new / spoke up" (direct signal)
    #    CHECKBOX: parent obs + child "asked questions" (proxy signal)
    s["Confidence & Active Participation"] = avg_pct(
        checkbox(['tried something new, spoke up']),
        checkbox(['asked questions'])
    )

    # 7. Social Inclusion & Belonging
    #    CHECKBOX: parent obs "appeared comfortable or settled"
    #    CHECKBOX: child emotion "connected to others"
    s["Social Inclusion & Belonging"] = avg_pct(
        checkbox(['appeared comfortable or settled']),
        checkbox(['connected to others'])
    )

    # 8. Positive Theatre Memory
    #    CHECKBOX: parent obs smiled/laughed/spoke about enjoyment
    #    CHECKBOX: parent obs talked positively afterwards
    s["Positive Theatre Memory"] = avg_pct(
        checkbox(['smiled, laughed or spoke about something they enjoyed']),
        checkbox(['talked positively about something'])
    )

    # 9. Well-being Through Arts
    #    SCALE 1-10: star rating >= 8 out of ALL 58 families
    s["Well-being Through Arts"] = avg_pct(
        scale(['how many stars would you give'], threshold=8)
    )

    # ── Social: Horizon (indicators 10–12) ──────────────────────────────────

    # 10. Equity of Cultural Access
    #    YES/NO: first-time access + subsidised tickets (free/partner)
    free_n = int((df[_cols(['how did you receive your tickets'])][0]
                  .astype(str).str.lower().str.contains('free|partner|subsidis')
                  ).sum()) if _cols(['how did you receive your tickets']) else 0
    ft_col_eq = _cols(['first live theatre'])
    ft_yes_eq = int(df[ft_col_eq].apply(
        lambda s: s.astype(str).str.strip().str.lower() == 'yes'
    ).any(axis=1).sum()) if ft_col_eq else 0
    equity_pct = round(max(ft_yes_eq, free_n) / n * 100, 1)
    s["Equity of Cultural Access"] = avg_pct(equity_pct)

    # 11. Lifelong Empathy & Life Skills
    #    CHECKBOX: Kind + Brave + Connected emotions
    s["Lifelong Empathy & Life Skills"] = avg_pct(
        checkbox(['kind', 'brave', 'connected to others'])
    )

    # 12. Community Social Capital
    #    CHECKBOX: comfortable + connected (same as Social Inclusion)
    s["Community Social Capital"] = avg_pct(
        checkbox(['appeared comfortable or settled']),
        checkbox(['connected to others'])
    )

    # ── Cultural: Spark (indicators 13–15) ──────────────────────────────────
    c = {}

    # 13. Cultural Identity Validation
    #    YES/NO: child "feel like you" + CHECKBOX: parent obs "felt similar"
    c["Cultural Identity Validation"] = avg_pct(
        yes_no(['feel a bit like you']),
        checkbox(['said they felt similar to a character or recognised'])
    )

    # 14. Creative Making Interest
    #    CHECKBOX: draw / make art / craft (subset of creative activities)
    c["Creative Making Interest"] = avg_pct(
        checkbox(['draw or make a story', 'make some art', 'make art or craft'])
    )

    # 15. Theatre Curiosity & Engagement
    #    CHECKBOX: child Curious emotion
    #    CHECKBOX: parent obs Watched closely
    #    CHECKBOX: child chose "think about the story" post-show
    c["Theatre Curiosity & Engagement"] = avg_pct(
        checkbox(['curious']),
        checkbox(['watched closely']),
        checkbox(['think about the story'])
    )

    # ── Cultural: Growth (indicators 16–18) ─────────────────────────────────

    # 16. Theatre Appreciation & Advocacy
    #    SCALE 1-10: NPS recommend >= 9 (high bar for advocacy)
    c["Theatre Appreciation & Advocacy"] = avg_pct(
        scale(['recommend'], threshold=9)
    )

    # 17. Cultural Literacy & Openness
    #    CHECKBOX: parent obs "commented on another culture"
    #    YES/NO:   child speaks language other than English
    c["Cultural Literacy & Openness"] = avg_pct(
        checkbox(['commented on something new they noticed about another culture']),
        yes_no(['language other than english'])
    )

    # 18. Repeat Attendance & Audience Growth
    #    YES/NO inverted: "Has the young person attended before?" → No = new audience
    c["Repeat Attendance & Audience Growth"] = avg_pct(
        no_yn(['has the young person attended a monkey baa show before'])
    )

    # ── Cultural: Horizon (indicators 19–21) ────────────────────────────────

    # 19. Lifelong Arts Engagement
    #    SCALE 1-10: recommend >= 9 (same signal as advocacy — intent to return)
    c["Lifelong Arts Engagement"] = avg_pct(
        scale(['recommend'], threshold=9)
    )

    # 20. Australian Storytelling Contribution
    #    YES/NO: child felt recognised in story
    c["Australian Storytelling Contribution"] = avg_pct(
        yes_no(['feel a bit like you'])
    )
...
def page_insights():
    ...
    # ── CHART A: Social Indicators – Spark ────────────────────────────────────
    st.markdown("#### 🧠 Social indicators – audience response")
    st.caption("Social: Spark (indicators 1–4)")

    spark_keys = [
        "Spontaneous Joy Response", "Creative Inspiration Spark",
        "Story Self-Recognition", "First-Time Theatre Access",
    ]
    spark_vals = {k: round(soc.get(k, 0), 1) for k in spark_keys}  # scores are already %

    if any(v > 0 for v in spark_vals.values()):
        spark_sorted = sorted(spark_vals.items(), key=lambda x: x[1])
        spark_df = pd.DataFrame(spark_sorted, columns=["Indicator", "Pct"])
        spark_df["Label"] = spark_df["Pct"].apply(lambda v: f"{v:.0f}%")
        fig_spark = px.bar(
            spark_df, x="Pct", y="Indicator", orientation="h",
            text="Label",
            range_x=[0, 110],
        ...
        fig_spark.update_traces(
            ...
            marker_color=["#185FA5" if v >= 70 else "#85B7EB" for v in spark_df["Pct"]],
            cliponaxis=False,
        )
        fig_spark.update_layout(
            ...
            xaxis=dict(title="% of respondents", gridcolor="#dbeafe", gridwidth=1,
                       tickvals=[0,25,50,75,100], ticktext=["0%","25%","50%","75%","100%"]),
            yaxis=dict(tickfont=dict(size=12)),
        )
        st.plotly_chart(fig_spark, use_container_width=True, config={"displayModeBar": False})

        top_spark = max(spark_vals, key=spark_vals.get)
        low_spark = min(spark_vals, key=spark_vals.get)
        col_sp1, col_sp2, col_sp3 = st.columns(3)
        col_sp1.metric("⬆ Highest", top_spark[:28], f"{spark_vals[top_spark]:.0f}%")
        col_sp2.metric("⬇ Needs attention", low_spark[:28], f"{spark_vals[low_spark]:.0f}%")
        col_sp3.metric("👥 Respondents", "Spark group", f"{n} families")

        top_pct  = spark_vals[top_spark]
        low_pct  = spark_vals[low_spark]
        ft_spark = spark_vals.get("First-Time Theatre Access", 0)
        st.info(
            f"ℹ️ **{top_spark}** is the standout Spark indicator at **{top_pct:.0f}%** of respondents — "
            f"direct evidence that the show is meeting its core mission of igniting curiosity and self-recognition "
            f"in young audiences. **{ft_spark:.0f}%** of families were experiencing live professional theatre for "
            f"the first time, confirming strong access impact. "
            f"**{low_spark}** scores lowest at {low_pct:.0f}% — this reflects limited post-show creative activity "
            f"questions in the survey rather than a programme gap, and should be addressed with richer questions in future years."
        )
    else:
        st.info("Upload survey data to calculate Spark indicator scores.")

    st.markdown("---")

    # ── CHART B: Social Indicators – Growth ───────────────────────────────────
    st.markdown("#### 🌱 Social indicators – audience response")
    st.caption("Social: Growth (indicators 5–9)")

    growth_keys = [
        "Empathy & Emotional Intelligence", "Confidence & Active Participation",
        "Social Inclusion & Belonging", "Positive Theatre Memory", "Well-being Through Arts",
    ]
    growth_vals = {k: round(soc.get(k, 0), 1) for k in growth_keys}  # scores are already %

    if any(v > 0 for v in growth_vals.values()):
        growth_sorted = sorted(growth_vals.items(), key=lambda x: x[1])
        growth_df = pd.DataFrame(growth_sorted, columns=["Indicator", "Pct"])
        growth_avg = round(sum(growth_vals.values()) / len(growth_vals), 1)
        growth_df["Label"] = growth_df["Pct"].apply(lambda v: f"{v:.0f}%")
        hi_col = "#0F6E56"; lo_col = "#5DCAA5"
        fig_growth = px.bar(
            growth_df, x="Pct", y="Indicator", orientation="h",
            text="Label",
            range_x=[0, 115],
            color_discrete_sequence=[hi_col],
        )
        fig_growth.update_traces(
            textposition="outside",
            textfont=dict(size=12, color="#1e293b"),
            marker_color=[hi_col if v >= growth_avg else lo_col for v in growth_df["Pct"]],
            cliponaxis=False,
        )
        fig_growth.add_vline(
            x=growth_avg, line_dash="dot", line_color="#f59e0b", line_width=2,
            annotation_text=f"Avg {growth_avg:.0f}%",
            annotation_position="top right",
            annotation_font=dict(size=11, color="#92400e"),
        )
        fig_growth.update_layout(
            height=300, margin=dict(l=10, r=70, t=20, b=40),
            paper_bgcolor="white", plot_bgcolor="#f0fdf4",
            font_family="DM Sans", font_size=12, showlegend=False,
            xaxis=dict(title="% of respondents", gridcolor="#d1fae5", gridwidth=1,
                       tickvals=[0,25,50,75,100], ticktext=["0%","25%","50%","75%","100%"]),
            yaxis=dict(tickfont=dict(size=12)),
        )
        st.plotly_chart(fig_growth, use_container_width=True, config={"displayModeBar": False})

        top_growth = max(growth_vals, key=growth_vals.get)
        low_growth = min(growth_vals, key=growth_vals.get)
        col_gr1, col_gr2, col_gr3 = st.columns(3)
        col_gr1.metric("⬆ Highest", top_growth[:28], f"{growth_vals[top_growth]:.0f}%")
        col_gr2.metric("📊 Group average", "Growth indicators", f"{growth_avg:.0f}%")
        col_gr3.metric("⬇ Focus area", low_growth[:28], f"{growth_vals[low_growth]:.0f}%")

        top_g_pct = growth_vals[top_growth]
        low_g_pct = growth_vals[low_growth]
        above_avg = [k for k, v in growth_vals.items() if v >= growth_avg]
        st.info(
            f"ℹ️ **{top_growth}** leads the Growth group at **{top_g_pct:.0f}%**, "
            f"with {len(above_avg)} of 5 indicators scoring above the group average of {growth_avg:.0f}%. "
            f"This confirms the programme is building lasting emotional and social connections beyond the initial spark. "
            f"**{low_growth}** scores lowest at {low_g_pct:.0f}% — the survey captures this through parent observations "
            f"of specific behaviours, which are naturally under-reported. Adding a direct question such as "
            f"'Did your child speak up more than usual?' in future surveys would strengthen this evidence."
        )
    else:
        st.info("Upload survey data to calculate Growth indicator scores.")
