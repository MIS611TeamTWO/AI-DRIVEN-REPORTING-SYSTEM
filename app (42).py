...
# ── Session state ─────────────────────────────────────────────────────────────
defaults = {
    'page': 'login', 'role': 'Laura Pike — Social Impact Manager',
...
def chat_response(question, ai, df):
    """Chat responses built entirely from real dashboard data — no hardcoded values."""
    q = question.lower().strip()
    n = len(df)
    cols = list(df.columns)

    # ── Pull real values from session state (set during Dashboard render) ────
    di = st.session_state.get('dashboard_insights', {})
    good_pct   = di.get('good_pct',  78)
    happy_pct  = di.get('happy_pct', 72)
    watch_pct  = di.get('watch_pct', 81)
    smiled_pct = di.get('smiled_pct',81)
    tried_pct  = di.get('tried_pct',  3)
    first_pct  = di.get('first_pct', 53)
    first_n    = di.get('first_n',   31)
    age56_pct  = di.get('age56_pct', 37)
    age34_pct  = di.get('age34_pct', 35)
    pct_rec9   = di.get('pct_rec9',  91)
    avg_stars  = di.get('avg_stars', 9.5)

    # ── Emotions from actual data ────────────────────────────────────────────
    def _fam_pct_chat(kws):
        matched = [c for c in cols if any(c.lower().startswith(k.lower()) for k in kws)]
        if not matched: return 0
        return round(df[matched].notna().any(axis=1).sum() / n * 100)

    excited_pct  = _fam_pct_chat(['Excited'])
    curious_pct  = _fam_pct_chat(['Curious'])
    proud_pct    = _fam_pct_chat(['Proud'])
    brave_pct    = _fam_pct_chat(['Brave'])

    # ── Parent observation columns ───────────────────────────────────────────
    def _obs_pct(kw):
        matched = [c for c in cols if kw.lower() in c.lower()]
        if not matched: return 0
        return round(df[matched].notna().any(axis=1).sum() / n * 100)

    talked_pct   = _obs_pct('talked positively')
    comfortable_pct = _obs_pct('appeared comfortable')
    similar_pct  = _obs_pct('said they felt similar')
    culture_pct  = _obs_pct('commented on something new')

    # ── All indicator scores (are already percentages) ───────────────────────
    all_scores = {**ai.get('social_indicators',{}), **ai.get('cultural_indicators',{})}
    low_ind  = min(all_scores.items(), key=lambda x: x[1]) if all_scores else ("Confidence & Active Participation", 18)
    low_name, low_score = low_ind

    age_early = age56_pct + age34_pct

    # ── How did children feel? ────────────────────────────────────────────────
    if any(w in q for w in ['children feel','child feel','how did','emotion','feel','happy','good inside',
                             'excited','joy','curious','emotion','emotional']):
        return (
            f"Based on {n} family responses, <strong>Good inside</strong> was the most selected emotion at "
            f"<strong>{good_pct}%</strong> of families, followed by <strong>Happy ({happy_pct}%)</strong> and "
            f"<strong>Excited ({excited_pct}%)</strong>. Curious was selected by {curious_pct}% and Proud by {proud_pct}% — "
            f"together these confirm the show created a warm, positive and safe emotional experience for young audiences. "
            f"Only 2% of families reported a child feeling Sad or Bored, showing overwhelmingly positive responses."
        )

    # ── What did parents observe? ─────────────────────────────────────────────
    if any(w in q for w in ['parent','observe','behaviour','behavior','watch','smile','laugh','tried',
                             'spoke up','comfortable','closely','talked']):
        return (
            f"Parent observations across {n} families show <strong>Watched closely ({watch_pct}%)</strong> and "
            f"<strong>Smiled/laughed ({smiled_pct}%)</strong> as the top two behaviours — both confirming active "
            f"engagement during the show. <strong>Talked positively afterwards ({talked_pct}%)</strong> is the strongest "
            f"post-show signal. Only <strong>{tried_pct}%</strong> of parents observed deeper behavioural change "
            f"(trying something new, speaking up more) — this is the weakest area and the highest-value outcome "
            f"to target in future programme planning."
        )

    # ── First-time / access ───────────────────────────────────────────────────
    if any(w in q for w in ['first time','first-time','first live','access','new audience',
                             'returning','repeat','theatre unlimited']):
        return (
            f"<strong>{first_pct}% of attendees ({first_n} of {n} families)</strong> experienced live professional "
            f"theatre for the very first time — nearly 1 in 2. This is the most direct evidence of the Theatre "
            f"Unlimited access mission. {round(100-first_pct)}% were returning audiences. Converting the "
            f"{first_n} first-time families into returning visitors is the single highest-leverage action for "
            f"long-term cultural impact — even a 30% conversion rate would add ~{round(first_n*0.3)} new "
            f"returning families next season."
        )

    # ── Age groups ────────────────────────────────────────────────────────────
    if any(w in q for w in ['age','young','year old','3-4','5-6','early childhood','toddler','preschool']):
        return (
            f"The largest audience group is <strong>5–6 year olds at {age56_pct}%</strong>, followed closely by "
            f"<strong>3–4 year olds at {age34_pct}%</strong>. Together these two early childhood groups represent "
            f"<strong>{age_early}% of all attendees</strong> — confirming the programme is strongly reaching its "
            f"primary target audience. This age alignment matters because children aged 3–6 are in the most "
            f"formative window for arts engagement and emotional development."
        )

    # ── Weakest area / improvement ────────────────────────────────────────────
    if any(w in q for w in ['weak','lowest','gap','concern','worst','attention','improve','risk','problem']):
        return (
            f"The weakest area is <strong>{low_name} ({low_score}%)</strong> — only <strong>{tried_pct}%</strong> "
            f"of parents observed their child trying something new or speaking up more after the show. "
            f"Three actions would directly address this: (1) a post-show creative take-home pack for the "
            f"{first_n} first-time families, (2) a follow-up email 4 weeks later with a next-season offer, "
            f"and (3) adding a direct survey question — 'Did your child try something creative at home after the show?'"
        )

    # ── Strengths / highlights ────────────────────────────────────────────────
    if any(w in q for w in ['strength','best','top','highest','strong','leading','success','highlight']):
        return (
            f"The programme's strongest results across {n} families: <strong>Good inside at {good_pct}%</strong> "
            f"(most selected emotion), <strong>{watch_pct}% watched closely</strong> throughout the show, and "
            f"<strong>{pct_rec9}% of parents gave a 9 or 10/10 recommendation</strong> (average {avg_stars}/10 stars). "
            f"The {first_pct}% first-time access rate is particularly impressive — nearly 1 in 2 families "
            f"were experiencing live theatre for the first time."
        )

    # ── Recommendation score / advocacy ──────────────────────────────────────
    if any(w in q for w in ['recommend','nps','rating','stars','satisfaction','score','advocate']):
        return (
            f"<strong>{pct_rec9}%</strong> of parents and carers gave a recommendation score of 9 or 10 out of 10, "
            f"with an average star rating of <strong>{avg_stars}/10</strong> across {n} families. "
            f"This is the Theatre Appreciation & Advocacy indicator — and at {pct_rec9}% it confirms extremely "
            f"high community trust and word-of-mouth impact for Monkey Baa."
        )

    # ── Overall programme performance ─────────────────────────────────────────
    if any(w in q for w in ['programme performing','program performing','how is my','overall performance',
                             'how are we doing','performance overall','programme performance',
                             'how well','overall','summary']):
        # Determine performance tier from key signals
        strong_signals = sum([
            good_pct >= 70,
            watch_pct >= 75,
            first_pct >= 45,
        ...
        # All indicator scores for finding weakest
        all_scores_perf = {**ai.get('social_indicators',{}), **ai.get('cultural_indicators',{})}
        sorted_scores = sorted(all_scores_perf.items(), key=lambda x: x[1]) if all_scores_perf else []
        weakest_two = sorted_scores[:2] if len(sorted_scores) >= 2 else sorted_scores

        weak_str = " and ".join(
            f"<strong>{k}</strong> ({v}%)" for k, v in weakest_two
        ) if weakest_two else f"<strong>{low_name}</strong> ({low_score}%)"

        return (
            f"<strong>Overall Performance: {tier_color} {tier}</strong><br><br>"

            f"<strong>✔ Strengths</strong><br>"
            f"The programme is performing well, with high emotional impact — "
            f"<strong>{good_pct}%</strong> of families reported children feeling 'Good inside' "
            f"and <strong>{happy_pct}%</strong> 'Happy'. Audience engagement is strong: "
            f"<strong>{watch_pct}%</strong> of parents observed active watching throughout the show, "
            f"and <strong>{smiled_pct}%</strong> noted smiling or laughter. "
            f"The programme is reaching its target audience effectively — "
            f"<strong>{age_early}%</strong> of attendees were aged 3–6 years. "
            f"First-time access is a standout result: <strong>{first_pct}%</strong> of families "
            f"({first_n} of {n}) were experiencing live professional theatre for the very first time.<br><br>"

            f"<strong>⚠ Area for Improvement</strong><br>"
            f"{weak_str} score{'s' if len(weakest_two)==1 else ''} lowest across all indicators, "
            f"indicating an opportunity to deepen post-show engagement or add educational touchpoints "
            f"that reinforce these outcomes beyond the performance itself.<br><br>"

            f"<strong>🎯 Conclusion</strong><br>"
            f"Across {n} families, the programme is delivering its core mission of sparking joy, "
            f"building emotional connection, and expanding access to live theatre for early childhood audiences. "
            f"The data confirms the show is doing what it sets out to do — the growth opportunity lies in "
            f"converting strong in-show engagement into lasting post-show impact.<br><br>"

            f"<strong>🚀 Recommendations</strong><br>"
            f"1. Introduce a structured post-show resource (take-home creative activity or story prompt) "
            f"distributed to all {first_n} first-time families — this directly targets the weakest indicators "
            f"and strengthens the access-to-engagement pipeline.<br>"
            f"2. Add 1–2 targeted survey questions in 2027 to capture post-show behaviour change directly, "
            f"so outcomes currently scored as 'Weak' can be properly evidenced."
        )

    # ── Default ───────────────────────────────────────────────────────────────
    return (
        f"Based on {n} family survey responses: <strong>Good inside ({good_pct}%)</strong> was the top emotion, "
        f"<strong>{watch_pct}% watched closely</strong>, <strong>{first_pct}% were first-time theatregoers</strong>, "
        f"and <strong>{age_early}% of children were aged 3–6 years</strong>. "
        f"Ask me about emotions, parent observations, first-time access, age groups, or the weakest area."
    )
...
def page_login():
    ...
    with col2:
        ...
        role = st.radio(
            "Role",
            ["Laura Pike — Social Impact Manager", "Kevin du Preez — Executive Director"],
...
def page_cleaning():
    ...
    st.markdown(
        f'<div style="font-size:13px;color:#64748b;font-weight:500;margin-bottom:8px">'
...
def page_insights():
    ...
    if 'active_chart' not in st.session_state:
        st.session_state.active_chart = 'all'
    active_chart = st.session_state.active_chart

    # Show filter banner + reset when a specific chart is active
    if active_chart != 'all':
        chart_labels = {
            'emotions':     '😊 How did children feel?',
            'observations': '👀 What did parents observe?',
            'first_time':   '🎭 First-Time vs Repeat Attendees',
        ...
        with col_banner:
            st.markdown(
                ...
                f'padding:8px 14px;font-size:13px;color:#1e40af;font-weight:500">'
                f'📌 Showing: <strong>{chart_labels.get(active_chart, active_chart)}</strong>'
                f' — based on your chat question</div>',
                unsafe_allow_html=True
            )
        with col_reset:
            if st.button("Show all ✕", key="reset_chart_filter", use_container_width=True):
                st.session_state.active_chart = 'all'
                st.rerun()
    ...
    # ── CHART A: How did children feel? ─────────────────────────────────────
    if active_chart in ('all', 'emotions'):
    ...
    if emo_data:
        ...
        st.info(
            f"✅ **{top_emo['Emotion']}** was the most selected emotion at **{top_emo['Pct']:.0f}%** of families — "
    ...
    # ── CHART B: What did parents observe? ─────────────────────────────────
    if active_chart in ('all', 'observations'):
    ...
    if obs_data:
        ...
        st.info(
            f"✅ **{top_obs['Behaviour']}** and **{obs_df.iloc[-2]['Behaviour']}** were the strongest parent-observed "
    ...
    # ── CHART 2: First-Time vs Repeat ────────────────────────────────────────
    if active_chart in ('all', 'first_time'):
        ...
        st.info(f"✅ **{yes_pct}% of attendees ({yes_n} families) experienced live professional theatre for the very first time.** This is the most direct evidence of the Theatre Unlimited access mission — nearly 1 in 2 audience members were first-timers. Converting these {yes_n} families into returning audiences is the single highest-leverage action for long-term cultural impact.")
    ...
    if active_chart in ('all', 'age'):
        ...
        if age_col:
            ...
            if len(ages_num) > 0:
                ...
                st.info(f"✅ **Largest audience group: {dominant} ({dominant_pct}%, {dominant_n} children)** — confirms the programme is reaching its primary target audience of early childhood. {second['Age Group']} is the second largest group ({second['Pct']}%), together these two groups represent {round(dominant_pct+second['Pct'])}% of all attendees.")
        else:
...
def render_chat():
    ...
    with st.form("chat_form", clear_on_submit=True):
        ...
        if send and user_msg.strip():
            ...
            # ── Keyword → chart filter ────────────────────────────────────
            q_lower = user_msg.lower()
            if any(k in q_lower for k in ['children feel','emotion','feel','happy','good inside','excited','joy','curious']):
                st.session_state.active_chart = 'emotions'
            elif any(k in q_lower for k in ['parent','observe','behaviour','behavior','watch','smile','laugh','tried']):
                st.session_state.active_chart = 'observations'
            elif any(k in q_lower for k in ['first time','first-time','first live','access','new audience','returning']):
                st.session_state.active_chart = 'first_time'
            elif any(k in q_lower for k in ['age','young','children age','year old','3-4','5-6','early childhood']):
                st.session_state.active_chart = 'age'
            else:
                st.session_state.active_chart = 'all'
            st.rerun()
      ...
      <div style="background:white;border:1px solid #bbf7d0;border-radius:8px;padding:14px">
        ...
        <div style="font-size:12px;color:#374151;line-height:1.7">
          ...
          after the show?" — to capture this outcome properly in 2027.</div>
                        ...
                        st.error(f"Report rendering error: {_exec_err}")
                        report_for_pdf = f"<p>Error: {_exec_err}</p>"
                else:
                    st.markdown(
