...
def page_insights():
    ...
    if soc:
        soc_sorted = sorted(soc.items(), key=lambda x: x[1])          # ascending → top bar = highest
        soc_avg = round(sum(soc.values()) / len(soc), 1)
        labels  = [k for k, _ in soc_sorted]
        scores  = [round(v, 1) for _, v in soc_sorted]
        pcts    = [round(v / 10 * 100, 1) for _, v in soc_sorted]
        bar_colors = ['#2563eb' if v >= soc_avg else '#93c5fd' for v in scores]
        dual_labels = [f"{s}/10  ({p}%)" for s, p in zip(scores, pcts)]

        import plotly.express as _px
        soc_df = pd.DataFrame({'Indicator': labels, 'Score': scores,
                                'Label': dual_labels, 'Color': bar_colors})
        fig1 = _px.bar(soc_df, x='Score', y='Indicator', orientation='h',
                       text='Label', color='Color',
                       color_discrete_map={c: c for c in set(bar_colors)},
                       range_x=[0, 14])
        fig1.update_traces(textposition='outside', textfont=dict(size=11, color='#1e293b'),
                           cliponaxis=False)
        fig1.add_vline(x=soc_avg, line_dash='dot', line_color='#f59e0b', line_width=2,
                       annotation_text=f'Avg {soc_avg}', annotation_position='top right',
                       annotation_font=dict(size=11, color='#92400e'))
        fig1.add_vline(x=7.0, line_dash='dash', line_color='#94a3b8', line_width=1.5,
                       annotation_text='Sector 7.0', annotation_position='bottom right',
                       annotation_font=dict(size=10, color='#64748b'))
        fig1.update_coloraxes(showscale=False)
        fig1.update_layout(
            height=max(360, len(labels) * 44 + 80),
            margin=dict(l=10, r=170, t=30, b=40),
            paper_bgcolor='white', plot_bgcolor='#f8fafc',
            font_family='DM Sans', font_size=12, showlegend=False,
            xaxis=dict(title='Score out of 10', gridcolor='#e2e8f0', gridwidth=1,
                       tickvals=[0,2,4,6,8,10], ticktext=['0','2','4','6','8','10']),
            yaxis=dict(tickfont=dict(size=12)),
        )
        st.plotly_chart(fig1, use_container_width=True, config={"displayModeBar": False})
        top_s = labels[-1]; top_v = scores[-1]
        low_s = labels[0];  low_v = scores[0]
        col_s1, col_s2, col_s3 = st.columns(3)
        col_s1.metric("⬆ Highest", top_s[:30], f"{top_v}/10")
        col_s2.metric("📊 Average", "Social Indicators", f"{soc_avg}/10")
        col_s3.metric("⬇ Focus Area", low_s[:30], f"{low_v}/10")
    else:
    ...
    if cult_show:
        cult_sorted = sorted(cult_show.items(), key=lambda x: x[1])   # ascending
        cult_avg = round(sum(cult_show.values()) / len(cult_show), 1)
        c_labels  = [k for k, _ in cult_sorted]
        c_scores  = [round(v, 1) for _, v in cult_sorted]
        c_pcts    = [round(v / 10 * 100, 1) for _, v in cult_sorted]
        c_colors  = ['#16a34a' if v >= cult_avg else '#86efac' for v in c_scores]
        c_labels_dual = [f"{s}/10  ({p}%)" for s, p in zip(c_scores, c_pcts)]

        import plotly.express as _px
        cult_df = pd.DataFrame({'Indicator': c_labels, 'Score': c_scores,
                                 'Label': c_labels_dual, 'Color': c_colors})
        fig3 = _px.bar(cult_df, x='Score', y='Indicator', orientation='h',
                       text='Label', color='Color',
                       color_discrete_map={c: c for c in set(c_colors)},
                       range_x=[0, 14])
        fig3.update_traces(textposition='outside', textfont=dict(size=11, color='#1e293b'),
                           cliponaxis=False)
        fig3.add_vline(x=cult_avg, line_dash='dot', line_color='#f59e0b', line_width=2,
                       annotation_text=f'Avg {cult_avg}', annotation_position='top right',
                       annotation_font=dict(size=11, color='#92400e'))
        fig3.add_vline(x=7.0, line_dash='dash', line_color='#94a3b8', line_width=1.5,
                       annotation_text='Sector 7.0', annotation_position='bottom right',
                       annotation_font=dict(size=10, color='#64748b'))
        fig3.update_coloraxes(showscale=False)
        fig3.update_layout(
            height=max(360, len(c_labels) * 44 + 80),
            margin=dict(l=10, r=170, t=30, b=40),
            paper_bgcolor='white', plot_bgcolor='#f0fdf4',
            font_family='DM Sans', font_size=12, showlegend=False,
            xaxis=dict(title='Score out of 10', gridcolor='#d1fae5', gridwidth=1,
                       tickvals=[0,2,4,6,8,10], ticktext=['0','2','4','6','8','10']),
            yaxis=dict(tickfont=dict(size=12)),
        )
        st.plotly_chart(fig3, use_container_width=True, config={"displayModeBar": False})
        top_c = c_labels[-1]; top_cv = c_scores[-1]
        low_c = c_labels[0];  low_cv = c_scores[0]
        col_c1, col_c2, col_c3 = st.columns(3)
        col_c1.metric("⬆ Highest", top_c[:30], f"{top_cv}/10")
        col_c2.metric("📊 Average", "Cultural Indicators", f"{cult_avg}/10")
        col_c3.metric("⬇ Focus Area", low_c[:30], f"{low_cv}/10")
    else:
