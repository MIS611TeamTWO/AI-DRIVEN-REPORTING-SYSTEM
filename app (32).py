...
def page_insights():
    ...
    st.session_state['dashboard_insights'] = {
        ...
        'pos_emo_pct': int(round(
            # % of families where ≥1 child selected ANY positive emotion
...
def render_chat():
    ...
    # ── Bottom navigation: Back + Proceed ───────────────────────────────────
    st.markdown("---")
    col_back_i, col_proc_i = st.columns([1, 3])
    with col_back_i:
        if st.button("← Back", key="back_insights_bottom", use_container_width=True):
            go('cleaning')
    with col_proc_i:
        if st.button("Proceed to Generate Reports →", key="proceed_reports", use_container_width=True):
            go('report')
